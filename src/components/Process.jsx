    ...
    <section className="py-12 px-5 lg:px-14 bg-gray-50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
        {steps.map((step, idx) => (
          ...
          >
            {/* Circle icon */}
            <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-red-500 w-16 h-16 mt-2.5 md:mt-0 flex items-center justify-center rounded-full shadow-lg group-hover:scale-110 transition">
              {step.icon}
            </div>

            {/* Step number */}
            <span className="absolute top-4 right-4 text-sm font-semibold text-gray-300">
              Step {step.id}
            </span>
